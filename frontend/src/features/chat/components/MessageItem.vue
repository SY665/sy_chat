    ...
    <article class="flex w-full gap-3" :class="isUser ? 'justify-end' : 'justify-start'">
        ...
        <div class="flex min-w-0 max-w-[80%] flex-col gap-1.5" :class="isUser ? 'items-end' : 'items-start'">
            ...
            <div v-else class="w-full min-w-0">
                ...
                <ImageGenerationStatus v-if="message.toolEvents?.length" :events="message.toolEvents" />

                <GeneratedImageResult v-for="image in generatedImages" :key="`${message.id}-${image.url}`"
                    :image="image" />
